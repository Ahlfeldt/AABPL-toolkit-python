...
def nested_distance_checks(
    ...
    # if cell cntd in disk return cell sum
...
def aggreagate_point_data_to_disks_vectorized_nested(
    ...
    id_y_mult=grid.id_y_mult
    cells_cntd_in_all_disks =grid.search.cells_cntd_in_all_disks
    ...
    for pt_id, a_pt_ycoord, a_pt_xcoord, cell_id, cell_region_id in zip(
        ...
        if not (grid_id_y, grid_id_x) == last_grid_id:
            ...
            cntd_cell_ids = [cell_id for cell_id in (id_y_mult*(cells_cntd_in_all_disks[:,0]+(grid_id_y))+(
                cells_cntd_in_all_disks[:,1]+grid_id_x)) if cell_id in nested_grid] 
       
            cntd_cells_sums = (_np_array([nested_grid[g_id]['sums'] for g_id in cntd_cell_ids]).sum(axis=0) 
                                      if len(cntd_cell_ids)>0 else zero_sums)
        #
            
        if not (grid_id_y, grid_id_x) == last_grid_id or last_cell_region_id != cell_region_id:
            # if cell changed or cell region changed
            cell_ids_full_in_cell_region = [cell_id for cell_id in (id_y_mult*(region_id_to_cntd_cells[cell_region_id][:,0]+(grid_id_y))+(
                region_id_to_cntd_cells[cell_region_id][:,1]+grid_id_x)) if cell_id in nested_grid]
                           
            cntd_cells_sums_region = (_np_array([nested_grid[g_id]['sums'] for g_id in cell_ids_full_in_cell_region if g_id in nested_grid]).sum(axis=0) 
                                             if len(cell_ids_full_in_cell_region)>0 else zero_sums)

            cell_ids_maybe_in = [cell_id for cell_id in (id_y_mult*(region_id_to_ovlpd_cells[cell_region_id][:,0]+(grid_id_y))+(
                region_id_to_ovlpd_cells[cell_region_id][:,1]+grid_id_x)) if cell_id in nested_grid]
        #
        ...
        sums_full_disk = cntd_cells_sums + cntd_cells_sums_region + overlapping_cells_sums
