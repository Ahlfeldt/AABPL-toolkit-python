...
################ DiskSearchTarget ######################################################################################
class DiskSearchTarget(DiskSearchBase):
    @time_func_perf
    def __init__(
        self,
        grid,
...
class DiskSearch(object):
    ...
    def update_search_params(
        ...
        (
        self.cells_cntd_in_all_disks, 
        self.cells_cntd_in_all_trgl_disks, 
        ...
        # TODO lvl is hard coded to 0. IF MULTIPLE LEVELS ARE CONSIDERED THIS MIGHT NEED TO BE ADJUSTED in classify_disk_cells
        self.cells_cntd_in_all_disks = [(0,(row,col)) for (row,col) in arr_to_tpls(self.cells_cntd_in_all_disks,int)]
    #
