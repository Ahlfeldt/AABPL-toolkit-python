...
from aabpl.utils.cell_geometry import ( min_dist_points_to_cell, max_dist_points_to_cell,)
...
def _visualize(weak_order_branch, indent=0): 
    ...
    for person in weak_order_branch:
        print(" - "*indent + str(person.id)+":"+
              '(oc'+ str(person.check_if_ovlpd_nr if hasattr(person, "check_if_ovlpd_nr") else None)+","+
              'cc'+str(person.check_if_cntd_nr if hasattr(person, "check_if_cntd_nr") else None)+')')
...
class WeakOrderTreeBranch(object):
    ...
    def add_attributes_to_branch(
        ...
        self.is_ovlpd_by_all_trgl1_disks = cell_to_all_int[self.id]
        self.is_never_cntd_in_trgl1_disks = cell_to_nev_cnt[self.id]
        
        if not self.is_ovlpd_by_all_trgl1_disks: 
            self.check_if_ovlpd_nr = tree.check_if_ovlpd_nrs.pop(0)
            tree.check_if_ovlpd_order.append(self.id)
        if not self.is_never_cntd_in_trgl1_disks: 
            self.check_if_cntd_nr = tree.check_if_cntd_nrs.pop(0)
            tree.check_if_cntd_order.append(self.id)
        #
...
class WeakOrderTree(object):
    ...
    def add_attributes_to_tree(
        ...
        """
        self.n_checks_if_ovlpd = len(trgl_always_overlaps_cells)-sum(trgl_always_overlaps_cells)
        self.n_checks_if_cntd = len(trgl_never_contains_cells)-sum(trgl_never_contains_cells)

        self.cells_ovlpd_by_all_trgl1_disks = [(row,col) for ((row,col),all_vtx_int) in zip(cells, trgl_always_overlaps_cells) if all_vtx_int]
        self.cells_cntd_in_no_trgl1_disk = [(row,col) for ((row,col),no_vtx_cnt) in zip(cells, trgl_never_contains_cells) if no_vtx_cnt]
        ...
        self.check_if_ovlpd_order = []
        self.check_if_cntd_order = []
        self.check_if_ovlpd_nrs = list(range(self.n_checks_if_ovlpd))
        self.check_if_cntd_nrs = list(range(self.n_checks_if_cntd))
        ...
        self.check_if_ovlpd_order = _np_array(self.check_if_ovlpd_order)
        self.check_if_cntd_order = _np_array(self.check_if_cntd_order)
    #  
#
        ...
        # in the triangle there might be cells that are never ovlpd and cells that are alwas included. those cells never have to be checked again.  
...
def recursive_cell_region_inference(
    ...
    Checks whether closest vertex of cell is within disk of specified radius, i.e. cell is at least partly cntd in disk. 
    For all points for which this hold true, it checks whether furthest vertex is also in disk, i.e. cell is fully cntd in disk.
    Result are stored in a 2D-matrix (number of points x number of cell to be checked) for whether cell in fully cntd in disk [disks_by_cells_contains]. 
    If cell is NOT fully cntd but partly respective values in partly overlap 2D-matrix are set to true [disks_by_cells_overlaps].
    The right column of both matrix is determined by first element fo remaining check_nrs.  

    For the subset of pts of which their disk have at least partly cntd the checked cell the function will be called recursively to test the next cell in the hierarchy.   
    ...
    for person in family_tree_pos:
        ...
        #     print(person.id,"CELL 3,3: ",person.is_ovlpd_by_all_trgl1_disks, person.is_never_cntd_in_trgl1_disks, len(reference_ids))
        # indicates column where result need to be stored into 
        # in the top level of tree all points are at least partly cntd by definition of the tree. thus this step will be skipped.
        if not person.is_ovlpd_by_all_trgl1_disks: 
            # check whether closest vertex is cntd in search disk
            check_overlap = (
        ...
        if not person.is_never_cntd_in_trgl1_disks: # potential speed looking up in full family_tree (where children may have >1 parents). If no children then cell will never be fully contains
            # check whether farthest vertex of cell is in disk. if true cell is fully cntd in disk.
            ...
            disks_by_cells_contains[reference_ids[check_contains], person.check_if_cntd_nr] = True
            # mark 
            if not person.is_ovlpd_by_all_trgl1_disks:
                disks_by_cells_overlaps[reference_ids[_np_invert(check_contains)], person.check_if_ovlpd_nr] = True
        else:
            if not person.is_ovlpd_by_all_trgl1_disks:
                disks_by_cells_overlaps[reference_ids, person.check_if_ovlpd_nr] = True
        #   
