...
from ..utils.misc import ( flatten_list, visualize, depth, list_dict_keys, dist_line_segment_to_point)
...
def classify_disk_cells(
        grid_spacing:float=250,
        r:float=750,
        include_boundary:bool=False,
        return_type:str=[None, 'cntd_by_all', 'cntd_by_trgl', 'ovlpd_by_all', 'ovlpd_by_trgl'][0]
    ) -> tuple:
    """
    ...
    # compare farthest vertex of cells: bool if cell is in cntd in disk of radius centered around any pt within grid cell 
    cell_is_cntd_in_all_disks = grid_spacing * max_possible_dist_cells_to_cell(unassigned_cells_in_max_steps_square) <= r
    cells_cntd_in_all_disks = unassigned_cells_in_max_steps_square[cell_is_cntd_in_all_disks,:]
    
    if 'cntd_by_all' == return_type:
        return cells_cntd_in_all_disks
    
    # those cells should not be part of other sets
    unassigned_cells_in_max_steps_square = unassigned_cells_in_max_steps_square[_np_invert(cell_is_cntd_in_all_disks),:]
    ...
    if 'ovlpd_by_all' == return_type:
        return cells_maybe_overlapping_a_disk
    
    # compare farthest vertex of cells: bool if cell is in cntd in disk of radius centered around any pt within triangle 1 
    cell_is_cntd_in_all_trgl_disks = [grid_spacing * max_possible_dist_trgl1_to_cell(cell) <= r for cell in unassigned_cells_in_max_steps_square]
    cells_cntd_in_all_trgl_disks = unassigned_cells_in_max_steps_square[cell_is_cntd_in_all_trgl_disks]    
    
    if 'cntd_by_trgl' == return_type:
        return cells_cntd_in_all_trgl_disks
    
    # those cells should not be in cells maybe overlapping disk around pts in triangle as they are surely contain
    unassigned_cells_in_max_steps_square = unassigned_cells_in_max_steps_square[_np_invert(cell_is_cntd_in_all_trgl_disks),:]
...
def classify_disk_cells_by_level(
        grid_spacing:float=250,
        r:float=750,
        nest_depth=0,
        include_boundary:bool=False,
    ...
    cell_is_cntd_in_all_disks = grid_spacing * max_possible_dist_cells_to_cell(unassigned_cells_in_max_steps_square) <= r
    cells_cntd_in_all_disks = unassigned_cells_in_max_steps_square[cell_is_cntd_in_all_disks,:]
    cells_cntd_in_all_disks_by_lvl = [(0,row_col) for row_col in cells_cntd_in_all_disks]
    
    # for each cell: if cntd add (lvl,cell) to cntd cells
    # if not cntd and lvl<nest_depth: check 4 subcells
...
def get_cells_by_lvl_ovlpd_by_cell_buffer(
        grid_spacing:float=250,
        r:float=750,
        nest_depth=0,
): 
    ...
    for row,col in unassigned_cells_in_max_steps_square:
        ...
        if farthest_dist <= r:
            ...
            ovlpd_cells.append((0,(row,col)))
    
    return cntd_cells, ovlpd_cells
...
def filter_cells_alw_cntd(
        grid_spacing:float=250,
        r:float=750,
        include_boundary:bool=False
    ) -> tuple:
    return classify_disk_cells(
       grid_spacing=grid_spacing, r=r, include_boundary=include_boundary, return_type='cntd_by_all'
    )
#
...
def filter_cells_alw_ovlpd(
        grid_spacing:float=250,
        r:float=750,
        include_boundary:bool=False
    ) -> tuple:
    return classify_disk_cells(
       grid_spacing=grid_spacing, r=r, include_boundary=include_boundary, return_type='ovlpd_by_trgl'
    )
#
...
def partition_cells_by_disk_relation(
        r:float=750,
        grid_spacing:float=750,
        count_only:bool=True
    ) -> tuple:
    """
    TODO this method and classify_disk_cells contain repeated logic. Only one shall be kept.
    ...
    - cntd within radius for all possible points in the cell
    - which cells might be overlaped, i.e. that are not fully cntd or partly 
    ...
    Args:
      r (float):
        ...
        this is used to check whether other grid cells are fully cntd in that disk or overlaped
    ...
    Returns:
      cell_info (tuple):
        tuple containg:
          ...
          - overlap_count (int):
            Number of cells that are cntd within disk around all point in cell
          - overlap_count (int):
            Number of cells that are overlap with disk around a point in cell but 
            not cntd in disk around all points of cell
        
        and if not count_only additionally containing:
          - overlap_ids (list):
            relative position of cells that are cntd within disk around all point in cell
          - overlap_ids (list):
